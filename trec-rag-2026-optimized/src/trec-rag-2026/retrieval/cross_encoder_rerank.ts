...
const STOPWORDS = new Set(
  "a an the and or but of to in on for with as at by from is are was were be been being this that these those it its into about how what why when where who which does do did can could should would will shall not no".split(
    " ",
  ),
);
...
/**
 * Instead of blindly taking the first N characters of a document (which may cut off before any
 * query-relevant content), find where the query's keywords actually occur in the document and
 * extract a window of context centered on the densest cluster of matches -- similar to how a
 * human skims a search result with Ctrl+F and reads the surrounding paragraph, not just the top
 * of the page. Falls back to the plain head of the document when no keyword occurs at all.
 */
export function extractRelevantSnippet(query: string, docText: string, windowChars = 2000): string {
  if (docText.length <= windowChars) return docText;
  const keywords = extractQueryKeywords(query);
  if (keywords.length === 0) return docText.slice(0, windowChars);

  const lowerDoc = docText.toLowerCase();
  const matchPositions: number[] = [];
  for (const kw of keywords) {
    let idx = lowerDoc.indexOf(kw);
    while (idx !== -1) {
      matchPositions.push(idx);
      idx = lowerDoc.indexOf(kw, idx + kw.length);
    }
  }
  if (matchPositions.length === 0) return docText.slice(0, windowChars);
  matchPositions.sort((a, b) => a - b);

  // Slide the window across match positions only (not every character) and count how many
  // matches fall inside each candidate window; keep the window with the most keyword hits.
  const half = Math.floor(windowChars / 2);
  let bestCenter = matchPositions[0];
  let bestCount = -1;
  let left = 0;
  for (let i = 0; i < matchPositions.length; i++) {
    const center = matchPositions[i];
    while (matchPositions[left] < center - half) left++;
    let right = left;
    let count = 0;
    while (right < matchPositions.length && matchPositions[right] <= center + half) {
      count++;
      right++;
    }
    if (count > bestCount) {
  ...
  const start = Math.max(0, Math.min(docText.length - windowChars, bestCenter - half));
  return docText.slice(start, start + windowChars);
}
...
export async function rerankWithCrossEncoder(
  ...
  for (const candidate of candidates) {
    const docText = extractRelevantSnippet(query, candidate.text, maxChars);
