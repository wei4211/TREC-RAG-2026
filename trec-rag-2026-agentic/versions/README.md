...
**變深度提交的成績幾乎等同固定 top-10** —— 目前的信心門檻(`ce_calibrated≥0.5`)
沒有額外拉高分數,變深度的價值在「符合官方規則」而非「分數更高」。
逐題檢視也抓到一個排序失敗:topic 515 有 414 篇相關文件,但融合排序前 4 名
（該題被壓到下限 k=4）一篇都沒中；追下去發現 BM25 排名 #0 的正確文件被
CrossEncoder(#58)和 dense(#51)兩票否決,融合後掉到 #24。這是三軌融合的
已知風險(多數暴力壓過單軌強訊號),留待之後處理,不影響本輪涵蓋線的結論。
