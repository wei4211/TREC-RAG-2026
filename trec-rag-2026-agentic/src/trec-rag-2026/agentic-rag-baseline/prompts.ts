...
export function buildAspectAnswerPrompt(input: { topic: TopicIdentity; aspect: string; documents: EvidenceDocumentForPrompt[]; alreadyWritten?: string[] }): string {
  ...
  return [
    ...
    "Write SHORT sentences: aim for about 15-20 words each, and state exactly ONE fact per sentence.",
    "Do NOT combine several facts into one long sentence. Two facts means two sentences. The answer is scored by how many distinct required facts it states completely, so one long compound sentence covers less than two short precise ones.",
    "State each fact COMPLETELY — include the specific entity, number, date, or mechanism so the claim stands alone without the surrounding text.",
    "Good (two sentences, each one complete fact): 'Women's national team players earned 38% of the per-game bonus paid to men in 2019.' / 'Media-rights deals accounted for 41% of league revenue that season.'",
    "Bad (one padded compound sentence): 'Persistent gender and racial pay gaps affect athlete compensation, and business interests shape both media-rights deals and team management.'",
