...
export function buildGroundedRevisionPrompt(
  ...
  return [
    ...
    "  - If no cited document supports the sentence at all, either weaken it to a supported statement or drop the sentence.",
...
export function buildAspectAnswerPrompt(input: { topic: TopicIdentity; aspect: string; documents: EvidenceDocumentForPrompt[]; alreadyWritten?: string[] }): string {
  ...
  return [
    ...
    "Prefer dense, enumerative sentences that pack several specific facts at once — name the specific groups, causes, figures, gaps, effects, or examples the evidence mentions rather than gesturing at them.",
    "Good: 'Persistent gender and racial pay gaps affect athlete compensation, and business interests shape both media-rights deals and team management.' Bad: 'Athlete compensation is a controversial and important topic.'",
...
export function buildAspectDecompositionPrompt(topic: TopicIdentity): string {
  return [
    ...
    "Rules: 4-8 aspects; each a short noun phrase (3-10 words); split the narrative FINELY so each aspect maps to a distinct sub-question a good answer must address; cover every distinct thing asked; do not overlap; do not add aspects the narrative does not ask about.",
    "",
    formatTopic(topic),
  ].join("\n");
}
